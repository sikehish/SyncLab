...
const Chatbot: React.FC = () => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-16 right-5 w-80 bg-white shadow-xl rounded-lg border border-gray-300 overflow-hidden">
          <div className="bg-blue-600 text-white p-3 text-lg font-semibold">Chatbot</div>

          <div className="h-64 p-3 overflow-y-auto space-y-2">
            {messages.map((msg, index) => (
              <div key={index} className={`p-2 rounded-lg text-white ${msg.sender === "user" ? "bg-blue-500 ml-auto" : "bg-gray-700 mr-auto"}`}>
                {msg.text}
              </div>
            ))}
          </div>
          ...
          <div className="p-3 flex items-center border-t">
            ...
            />
            <button onClick={sendMessage} className="bg-blue-600 p-2 rounded-r-lg text-white hover:bg-blue-700">
              <FaPaperPlane />
            </button>
          </div>
        </div>
